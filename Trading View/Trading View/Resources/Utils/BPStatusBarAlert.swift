...
public class BPStatusBarAlert: UIView {
    static let bpStatusBarAlert =  BPStatusBarAlert()
    ...
    fileprivate var statusBarHeight:CGFloat {
        return UIApplication.shared.statusBarFrame.size.height + 5
    }
...
extension BPStatusBarAlert {
    ...
    fileprivate func setupMessageLabel() {
        ...
        messageLabel.frame = CGRect(x: 0, y: statusBarHeight - deviceStatusBarHeight  + 5, width: frame.size.width, height: deviceStatusBarHeight)
        messageLabel.font = UIFont.systemFont(ofSize: 10, weight: .medium)

        if #available(iOS 11.0, *),
            let safeHeight = UIApplication.shared.keyWindow?.safeAreaInsets.top  {
            if safeHeight == 44
            {
                messageLabel.frame = CGRect(x: 0, y: statusBarHeight - deviceStatusBarHeight - 3, width: frame.size.width, height: deviceStatusBarHeight)
...
extension BPStatusBarAlert {
    ...
    fileprivate func addAlertViewInContainerWindow() {
        ...
        containerWindow?.windowLevel = UIWindow.Level(rawValue: 2)
